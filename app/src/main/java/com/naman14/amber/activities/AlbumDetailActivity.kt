 ...
 *   Time 2019/4/11
 **/
class AlbumDetailActivity : BaseActivity() , ATEActivityThemeCustomizer,  ATEToolbarCustomizer, ATEStatusBarCustomizer {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_album_detail)
        val id = intent.getLongExtra(Constants.ALBUM_ID, 0L)
        val t = supportFragmentManager.beginTransaction()
        t.add(R.id.fragment_container, AlbumDetailFragment.newInstance(id, false, null)).commit()
    }
